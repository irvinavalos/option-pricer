...
from tests.constants import HULL_ABS, PUT_CALL_PARITY_REL
...
def test_black_scholes_call_hull(hull_15: OptionTestCase):
    ...
    assert state.call_price() == pytest.approx(hull_15.expected_call, abs=HULL_ABS)


def test_black_scholes_put_hull(hull_15: OptionTestCase):
    """Test put price method matches Hull example 15.6"""
    state = hull_15.market.to_bs_state()
    assert state.put_price() == pytest.approx(hull_15.expected_put, abs=HULL_ABS)
...
def test_black_scholes_put_call_parity(bs_params):
    ...
    assert parity == pytest.approx(expected, rel=PUT_CALL_PARITY_REL)
