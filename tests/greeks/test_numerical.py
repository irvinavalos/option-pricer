...
from bspx.greeks import NumericalBackend
...
def test_delta_fd_matches_analytical(
    ...
    ana_backend = hull_19_delta.market.analytical_backend()
    num_backend = hull_19_delta.market.numerical_backend(black_scholes_price)
    delta_ana = ana_backend.delta(option_type)
    delta_num = num_backend.delta(option_type)
    assert delta_num == pytest.approx(delta_ana, rel=GREEK_ACC_REL)
...
def test_delta_fd_call_put_symmetry(bs_params):
    ...
    assert delta_call + np.abs(delta_put) == pytest.approx(1.0, abs=PUT_CALL_PARITY_REL)
...
def test_theta_fd_matches_analytical_calendar(
    ...
    ana_backend = hull_19_theta.market.analytical_backend()
    num_backend = hull_19_theta.market.numerical_backend(black_scholes_price)
    theta_ana = ana_backend.theta(option_type)
    theta_num = num_backend.theta(option_type)
    assert theta_num == pytest.approx(theta_ana, rel=GREEK_ACC_REL)
...
def test_theta_fd_matches_analytical_trading(
    ...
    ana_backend = hull_19_theta.market.analytical_backend()
    num_backend = hull_19_theta.market.numerical_backend(black_scholes_price)
    theta_ana = ana_backend.theta(option_type, day_count=DayCount.TRADING)
    theta_num = num_backend.theta(option_type, day_count=DayCount.TRADING)
    assert theta_num == pytest.approx(theta_ana, rel=GREEK_ACC_REL)
...
def test_gamma_fd_matches_analytical(hull_19_gamma: GammaTestCase):
    ana_backend = hull_19_gamma.market.analytical_backend()
    num_backend = hull_19_gamma.market.numerical_backend(black_scholes_price)
    gamma_ana = ana_backend.gamma()
    gamma_num = num_backend.gamma()
    assert gamma_num == pytest.approx(gamma_ana, rel=GREEK_ACC_REL)


def test_vega_fd_matches_analytical(hull_19_vega: VegaTestCase):
    ana_backend = hull_19_vega.market.analytical_backend()
    num_backend = hull_19_vega.market.numerical_backend(black_scholes_price)
    ana_vega = ana_backend.vega()
    num_vega = num_backend.vega()
    assert num_vega == pytest.approx(ana_vega, rel=GREEK_ACC_REL)


@pytest.mark.parametrize("option_type", ["call", "put"])
def test_rho_fd_matches_analytical(hull_19_rho: RhoTestCase, option_type: OptionType):
    ana_backend = hull_19_rho.market.analytical_backend()
    num_backend = hull_19_rho.market.numerical_backend(black_scholes_price)
    ana_rho = ana_backend.rho(option_type)
    num_rho = num_backend.rho(option_type)
    assert num_rho == pytest.approx(ana_rho, rel=GREEK_ACC_REL)
...
def test_rho_fd_call_put_relationship(bs_params):
    ...
    assert rho_call + np.abs(rho_put) == pytest.approx(scale, abs=PUT_CALL_PARITY_REL)
